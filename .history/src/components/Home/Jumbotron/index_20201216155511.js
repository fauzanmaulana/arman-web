...
function Jumbotron(props) {
    ...
    const hendleSearch = async e => {
        if(e.keyCode === 13){
            const response = await fetch(`${window.baseUrl}/api/contents/search?keyword=${keyword}&page=${page}`, {
                headers: new Headers({
                    ...
                    <div className="row align-items-center">
                        <div className="col-xl-12 col-md-11">
                            <div className="slider_text">
                                ...
                                <input className="jumbotron-input wow fadeInDown mt-3" placeholder="search article.." onKeyDown={e => hendleSearch(e)} onChange={() => setKeyword(e.target.value)} />
                            </div>
                        </div>
                    </div>
                </div>
