...
function Article(props) {
    ...
    const getExploreData = async () => {
        const response = await fetch('https://app.arman.id/api/contents/explore',{
            headers: new Headers({
        ...
        setInterestTopics(result.data.topics.filter(topic => topic))
    }
            ...
            <div className="row">
                <>
                    <div className="col-lg-8">
                        <ContentList item={props.bookmark}/>
                    </div>
                    <div className="col-lg-4" id="sidelist-wrapper">
                        <div className="sticky-top sidelist mb-5">
                            ...
                            <hr/>
                            {toogle ? <BookmarkSide item={props.bookmark}/> :  <Topics interest={interestTopics} all={allTopics} />}
                        </div>
                    </div>
                </>
            </div>
